...
function addRow(lastPosition, resourceID = '', resourceName = '', resourceType = '') {
    ...
    orderingHTML += 'id="sub' + nextRowNumber + 'Order" size="2" value="' + nextRowNumber + '" class="text-area-order" ';
    ...
    jQuery(html).appendTo(document.getElementById('subOrdinates').tBodies[0]);
}
...
function getSubOrdinates() {
    ...
    const count = jQuery('#subOrdinates').find('tr').length - 1;
    let current = [],
        index,
        order;
