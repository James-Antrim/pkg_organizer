...
class Instances extends ResourceHelper
{
    ...
    public static function getConditions(): array
    {
        ...
        $conditions['my']     = (!empty($conditions['userID']) and Input::bool('my'));

        $conditions['date'] = Dates::standardize(Input::cmd('date', date('Y-m-d')));

        $delta               = Input::integer('delta');
        ...
        if (empty($conditions['my'])) {
            ...
            if ($courseID = Input::integer('courseID')) {
            ...
            if ($personIDs = $personID ? [$personID] : Input::resourceIDs('personIDs')) {
            ...
            $roomID = Input::integer('roomID');
            if ($roomIDs = $roomID ? [$roomID] : Input::resourceIDs('roomIDs')) {
                $conditions['roomIDs'] = $roomIDs;
            }
            elseif ($room = Input::cmd('room') and $roomID = Rooms::getID($room)) {
                $conditions['roomIDs'] = [$roomID];
            }

            if ($subjectID = Input::integer('subjectID')) {
                $conditions['subjectIDs'] = [$subjectID];
            }

            $unitID = Input::integer('unitID');
            if ($unitIDs = $unitID ? [$unitID] : Input::resourceIDs('unitIDs')) {
