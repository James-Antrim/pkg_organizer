...
class Text extends Base
{
     ...
     */
    public static function getClassConstant(string $class): string
    {
        $parts          = preg_split('/([A-Z][a-z]+)/', $class, 0, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
    ...
    {
        // The key is in fact a key and the component preface is missing.
        if (preg_match('/^[A-Z_]+$/', $key) !== false and !str_starts_with($key, 'ORGANIZER')) {
            $key = "ORGANIZER_$key";
        }
