...
class Mapping extends BaseModel
{
	...
	public function checkForMapping($resourceID, $resourceType)
	{
		...
		$query->select('COUNT(*)')->from('#__organizer_mappings')->where("{$resourceType}ID = '$resourceID'");
	...
	public function deleteByResourceID($resourceID, $type)
	{
		...
		$mappingIDsQuery->select('id')->from('#__organizer_mappings');
		switch ($type)
		{
			case 'program':
	...
	{
		$mappingIDsQuery = $this->_db->getQuery(true);
		$mappingIDsQuery->select('id')->from('#__organizer_mappings')->where("parentID = '$mappingID'");
		$this->_db->setQuery($mappingIDsQuery);
		$mappingIDs = OrganizerHelper::executeQuery('loadColumn', []);
	...
	private function deleteEntry($entryID)
	{
		...
		$deleteQuery->delete('#__organizer_mappings')->where("id = '{$mapping['id']}'");
		...
		$siblingsQuery->update('#__organizer_mappings');
		...
		$updateLeftQuery->update('#__organizer_mappings');
		...
		$updateRightQuery->update('#__organizer_mappings');
	...
	private function determineLft($parentID, $ordering)
	{
		...
		$rgtQuery->select('MAX(rgt)')->from('#__organizer_mappings');
		...
		$lftQuery->select('lft')->from('#__organizer_mappings');
	...
	private function getChildren($resourceID, $type = 'pool', $deep = true)
	{
		...
		$existingQuery->select('id')->from('#__organizer_mappings');
		...
		if (!empty($firstID))
		{
			...
			$childrenQuery->from('#__organizer_mappings');
	...
	private function getOrdering($parentID, $resourceID, $type = 'pool')
	{
		...
		$existingOrderQuery->select('ordering')->from('#__organizer_mappings');
		...
		$maxOrderQuery->select('MAX(ordering)')->from('#__organizer_mappings')->where("parentID = '$parentID'");
	...
	private function getParent($parentID)
	{
		...
		$parentQuery->select('*')->from('#__organizer_mappings')->where("id = '$parentID'");
	...
	public function saveProgram($programID)
	{
		...
		if (empty($rootMapping))
		{
			...
			$leftQuery->select('MAX(rgt)')->from('#__organizer_mappings');
	...
	{
		$query = $this->_db->getQuery(true);
		$query->select('*')->from('#__organizer_mappings')->where("{$type}ID = '$resourceID'");
		$this->_db->setQuery($query);
	...
	{
		$query = $this->_db->getQuery(true);
		$query->update('#__organizer_mappings')->set('ordering = ordering + 1');
	...
	{
		$lftQuery = $this->_db->getQuery(true);
		$lftQuery->update('#__organizer_mappings')->set('lft = lft + 2')->where("lft >= '$value'");
		$this->_db->setQuery($lftQuery);
		$success = (bool) OrganizerHelper::executeQuery('execute');
		if (!$success)
		{
		...
		$rgtQuery = $this->_db->getQuery(true);
		$rgtQuery->update('#__organizer_mappings')->set('rgt = rgt + 2')->where("rgt >= '$value'");
		$this->_db->setQuery($rgtQuery);
