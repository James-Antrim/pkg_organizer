...
abstract class MergeModel extends BaseModel
{
	...
	/**
	 * Updates saved lesson configurations.
	 *
	 * @param   string  $context the update context (rooms|teachers)
	 *
	 * @return bool true on success, otherwise false
	 */
	protected function updateLessonConfigurations(string $context)
	{
		$mergeID = $this->selected[0];
		$relevantIDs = [];
		$query = $this->_db->getQuery(true);
		$query->select('id');
		$query->from('#__thm_organizer_lesson_configurations');
		$this->_db->setQuery($query);

		foreach ($this->selected as $resourceID)
		{
			$query->clear('where');
			$regexp = '"' . $context . '":\\{("[0-9]+":"[\w]*",)*"' . $resourceID . '"';
			$query->where("configuration REGEXP '$regexp'");
			$this->_db->setQuery($query);

			if ($results = Helpers\OrganizerHelper::executeQuery('loadColumn'))
			{
				$relevantIDs = array_merge($relevantIDs, $results);
			}
		}

		$relevantIDs = array_unique($relevantIDs);
		if (empty($relevantIDs))
		{
			return true;
		}

		foreach ($relevantIDs as $configID)
		{
			$table = new Tables\LessonConfigurations();
			if (!$table->load($configID))
			{
				continue;
			}

			$configuration = json_decode($table->configuration, true);
			$contextResources = $configuration[$context];
			$contextResourceIDs = array_keys($contextResources);

			foreach ($contextResourceIDs as $contextIndex)
			{
				// They don't matter anymore
				if ($contextResources[$contextIndex] === 'removed')
				{
					unset($contextResources[$contextIndex]);
				}

				if (in_array($contextIndex, $this->selected) and array_key_exists($contextIndex, $contextResources))
				{
					$oldDelta = $contextResources[$contextIndex];
					unset($contextResources[$contextIndex]);

					$contextResources[$mergeID] = $oldDelta;
				}
			}

			$configuration[$context] = $contextResources;
			$table->configuration = json_encode($configuration);
