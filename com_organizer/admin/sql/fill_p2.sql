...
# set prep course values by event and organization (12 = GI, 13 = ZDH, 14 = MNI, 19 = FB)
UPDATE `#__organizer_events` AS e
...
SET `campusID` = (SELECT DISTINCT `campusID`
                  FROM `#__thm_organizer_subjects` AS s
                           INNER JOIN `#__thm_organizer_subject_mappings` AS sm ON sm.`subjectID` = s.`id`
                  WHERE sm.`plan_subjectID` = e.`id`);
# endregion

# region subject_events
INSERT IGNORE INTO `#__organizer_subject_events` (id, eventID, subjectID)
SELECT `id`, `plan_subjectID`, `subjectID`
FROM #__thm_organizer_subject_mappings;
# endregion

# region event_coordinators
     INSERT IGNORE INTO `#__organizer_event_coordinators` (`eventID`, `personID`)
SELECT DISTINCT `plan_subjectID`, `teacherID` FROM `#__thm_organizer_subject_teachers` AS st
         INNER JOIN `#__thm_organizer_subject_mappings` AS sm ON sm.`subjectID` = st.`subjectID`
WHERE `teacherResp` = 1;
# endregion

# region schedules
INSERT IGNORE INTO `#__organizer_schedules` (id, organizationID, termID, userID, creationDate, creationTime,
                                             schedule, active)
SELECT `id`,
       `departmentID`,
       `planningPeriodID`,
...
# region instances
INSERT IGNORE INTO `#__organizer_instances`(`eventID`, `blockID`, `unitID`, `methodID`, `delta`, `modified`)
...
# reassociate prep course event associations
...
# remove events which are no longer associated with instances
DELETE
FROM `#__organizer_events`
...
SET `startDate` = (SELECT MIN(date)
                   FROM `#__organizer_blocks` AS b
                            INNER JOIN `#__organizer_instances` AS i ON i.`blockID` = b.`id`
                   WHERE i.`unitID` = u.`id`),
