...
const ScheduleApp = function (variables) {
    ...
    function ScheduleTable(schedule) {
        ...
        function insertInstances(instances) {
            const rows = table.getElementsByTagName('tbody')[0].getElementsByTagName('tr');
            let block, blockEnd, blockStart, blockTimes,
                cell,
                colNumber = variables.isMobile ? visibleDay : 1,
                index, elementIndex, event, eventElements, nextBlock,
                nextCell, nextRow, showOwnTime, tableStartTime, tableEndTime;

            if (timeGrid.periods) {
                insertInstanceswoGrid(instances);
                return;
            }
            ...
            for (index in instances) {
                ...
                for (block in instances[index]) {
                    if (!instances[index].hasOwnProperty(block)) {
                        continue;
                    }
                    ...
                    if (variables.userID && !isUserSchedule && isOccupiedByUserEvent(rowIndex, colNumber)) {
                        jQuery(cell).addClass('occupied');
                    }

                    for (event in instances[index][block]) {
                        if (!instances[index][block].hasOwnProperty(event)) {
                            continue;
                        }

                        showOwnTime = tableStartTime !== blockStart || tableEndTime !== blockEnd;
                        eventElements = createEvent(instances[index][block][event], showOwnTime);
         ...
         * @param {Object} events
         */
        function insertInstanceswoGrid(events) {
            const rows = table.getElementsByTagName('tbody')[0].getElementsByTagName('tr');
